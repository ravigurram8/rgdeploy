...
rgurl=$6
LOCALHOME=`pwd`
#rgurl="enterprise-test3.rlcatalyst.com"
#amiid="ami-0104e0699916e1ce2"
runid=$(openssl rand -hex 4)
echo "Beginning deployment $runid"
...
# Populate the new S3 bucket with RG Deployment files default bucketname rg-newdeployment-docs
...
userpoolclient_id=$(aws cloudformation describe-stack-resource --stack-name "$userpoolstackname" |\
    --logical-resource-id "CognitoUserPoolClient" | jq -r '.StackResources[] | .PhysicalResourceId')
#Capture User Pool ID
userpool_id=$(aws cloudformation describe-stack-resource --stack-name "$userpoolstackname" |\
    --logical-resource-id "CognitoUserPool" | jq -r '.StackResources[] | .PhysicalResourceId')

# Integrate the DocumentDB stack at this point.
dbstackname="RG-Portal-DB-$runid"
aws cloudformation deploy --template-file $LOCALHOME/rg-deployment-docs/documentdb_full_stack.yml \
                        --stack-name "$dbstackname" 
                        --parameter-overrides DBClusterName="rgdbcluster" DBInstanceName="rgdbinst" \
                                              MasterUser="rgadmin" MasterPassword="secret$runid" DBInstanceClass="db.t3.medium" 

#update the AMI id in the instance profile CFT
sed -i -e "s/ami-.*/$amiid/" $LOCALHOME/rg-deployment-docs/RGInstanceProfile.yml
...
aws cloudformation deploy --template-file $LOCALHOME/rg-deployment-docs/RGInstanceProfile.yml \
                        --stack-name "$mainstackname" 
                        --parameter-overrides ClientId="$userpoolclient_id" UserPoolId="$userpool_id" \
                                              CFTBucketName="$bucketname" RGUrl="$rgurl" UserPassword="appadmin" AdminPassword="dbadmin" \
...
myip=$(aws cloudformation describe-stack-resource --stack-name "$mainstackname" |\
    --logical-resource-id "RGPortalEC2Instance" | jq -r '.StackResources[] | .PhysicalResourceId')
